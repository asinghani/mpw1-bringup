...
ASM_CMD = "riscv-none-embed-gcc -march=rv32i -mabi=ilp32 -ffreestanding -nostartfiles -Wl,-Bstatic,--strip-debug"

def assemble_file(filename):
    cmd_build = f"{ASM_CMD} -o /tmp/rvbuild.elf {filename}"
    cmd_copy = f"riscv-none-embed-objcopy --only-section=.text /tmp/rvbuild.elf -O binary /tmp/rvbuild.bin"
